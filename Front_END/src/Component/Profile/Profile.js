...
const Profile = ({token, setToken}) => {
  ...
  const userData = {
    ...
    name: token?.name || "Guest", // Fallback to 'Guest' if name is unavailable
    email: token?.email || "Not provided", // Fallback if email is missing
  };
  ...
  try {
    const config = {
      headers: {
        ...
        'Authorization': `Bearer ${token.refreshToken}`, // Optional if stored in state
    ...
    // setToken(null); // Clear user state
